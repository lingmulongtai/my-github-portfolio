...
├── data/github.json                GitHub から自動取得（手で触らない）
├── scripts/sync-github.mjs         取得スクリプト
└── .github/workflows/sync-github.yml  毎日走らせる設定
```
...
1. リポジトリを作って、この4つ（＋`index.html`）を push
2. Settings → Pages → Source を **GitHub Actions** ではなく **Deploy from a branch**（`main` / `root`）に
3. Actions タブで `Sync GitHub data` を一度手動実行 → `data/github.json` が生成される

以降は毎日 3:00 JST に自動更新されます。`projects.json` を編集して push したときも走ります。
